                                               ...
                                               const gchar * const   *refs)
{
  ...
  repo_path = (repo_index != NULL) ? g_strdup_printf ("/%u/", g_variant_get_uint16 (repo_index)) : g_strdup ("/");
  ...
  for (i = 0; i < self->resolve_tasks->len; i++)
    {
      ...
      refs = g_task_get_task_data (task);
                                        ...
                                        gpointer             user_data)
{
  ...
  g_task_set_task_data (task, g_strdupv ((gchar **) refs), (GDestroyNotify) g_strfreev);
