...
test_repo_finder_mount_init (void)
{
  ...
  monitor = ostree_mock_volume_monitor_new (NULL);
...
static void
test_repo_finder_mount_no_mounts (void)
{
  g_autoptr(OstreeRepoFinderMount) finder = NULL;
  g_autoptr(GVolumeMonitor) monitor = NULL;
                                     ...
                                     gconstpointer  test_data)
{
  ...
  g_autoptr(GPtrArray) results = NULL;  /* (element-type OstreeRepoFinderResult) */
  g_autoptr(GError) error = NULL;
  g_autoptr(GList) volumes = NULL;  /* (element-type OstreeMockVolume)  */
  g_autoptr(OstreeMockVolume) volume_no_drive = NULL;
  g_autoptr(OstreeMockVolume) volume_no_mount = NULL;
  g_autoptr(OstreeMockVolume) volume_drive_not_removable = NULL;
  g_autoptr(OstreeMockVolume) volume_no_repos = NULL;
  g_autoptr(OstreeMockVolume) volume_repo1 = NULL;
  g_autoptr(OstreeMockVolume) volume_repo2 = NULL;
  g_autoptr(OstreeMockDrive) non_removable_drive = NULL;
  g_autoptr(OstreeMockDrive) removable_drive = NULL;
  ...
  non_removable_drive = ostree_mock_drive_new (FALSE);
  removable_drive = ostree_mock_drive_new (TRUE);
  ...
  non_removable_mount = ostree_mock_mount_new (non_removable_root);

  no_repos_root = g_file_get_child (fixture->tmp_dir, "no-repos");
  no_repos_repos = assert_create_repos_dir (no_repos_root);
  no_repos_mount = ostree_mock_mount_new (no_repos_root);
  ...
  volume_no_drive = ostree_mock_volume_new ("no-drive", NULL, G_MOUNT (non_removable_mount));
  volume_no_mount = ostree_mock_volume_new ("no-mount", G_DRIVE (non_removable_drive), NULL);
  volume_drive_not_removable = ostree_mock_volume_new ("drive-not-removable",
                                                       G_DRIVE (non_removable_drive),
                                                       G_MOUNT (non_removable_mount));
  volume_no_repos = ostree_mock_volume_new ("no-repos", G_DRIVE (removable_drive),
                                            G_MOUNT (no_repos_mount));
  volume_repo1 = ostree_mock_volume_new ("repo1", G_DRIVE (removable_drive),
                                         G_MOUNT (repo1_mount));
  volume_repo2 = ostree_mock_volume_new ("repo2", G_DRIVE (removable_drive),
                                         G_MOUNT (repo2_mount));

  volumes = g_list_prepend (volumes, volume_no_drive);
  volumes = g_list_prepend (volumes, volume_no_mount);
  volumes = g_list_prepend (volumes, volume_drive_not_removable);
  volumes = g_list_prepend (volumes, volume_no_repos);
  volumes = g_list_prepend (volumes, volume_repo1);
  volumes = g_list_prepend (volumes, volume_repo2);

  monitor = ostree_mock_volume_monitor_new (volumes);
  finder = ostree_repo_finder_mount_new (monitor);
...
int main (int argc, char **argv)
{
  ...
  g_test_add_func ("/repo-finder-mount/no-mounts", test_repo_finder_mount_no_mounts);
